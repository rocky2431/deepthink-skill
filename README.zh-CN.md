...
版本：0.2.1。插件包含 Skill、参考文档和模板，模型与工具由宿主提供。
...
持续议题沿用已有目录；新议题优先采用明确指定的路径和项目现有 `docs/` 或 `documents/` 约定。没有约定时使用 `documents/<议题>/`，既有 `thinking/` 文件不迁移。Agent 会在开始时说明实际路径，议题材料保存在可复用的插件包之外。
...
进度说明包括目前已定下什么及其依据、仍开放什么，以及下一步做什么。约定目的已有依据充分的答案时，Agent 主动交付。交付表示成果已经准备好，是否接受由你决定。
